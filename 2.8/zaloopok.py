#
#
#   Credits to nemyax for original script:
#   (https://sourceforge.net/p/blenderbitsbobs/wiki/Zaloopok/)
#   I'm just keeping an updated copy for 2.8
#
#
...
def addon_prefs():
    ...
    addon = prefs.addons[__name__]
    return addon.preferences

def view3d_panel_update(self, context):
    prefs = addon_prefs()
    if prefs.view3d_panel_location_enum == 'PROPERTIES':
        try:
            bpy.utils.unregister_class(ZaloopokView3DPanel)
            bpy.utils.register_class(ZaloopokPropertiesPanel)
        except:
            None
    else:
        try:
            bpy.utils.unregister_class(ZaloopokPropertiesPanel)
...
class ZaloopokView3DPanel(Panel):
    bl_category = "View"
    bl_context = "mesh_edit"
    bl_region_type = 'UI'
    bl_space_type = 'VIEW_3D'
    bl_idname = "MESH_PT_Zaloopok"
    bl_label = "Zaloopok"
...
class ZaloopokPropertiesPanel(Panel):
    bl_category = "Options"
...
class ZaloopokAddonPrefs(bpy.types.AddonPreferences):
    bl_idname = __name__

    view3d_panel_location_items = [
        ("SIDE", "Side Panel", "", 1),
        ("PROPERTIES", "Properties Panel", "", 2),]

    view3d_panel_location_enum : EnumProperty(
        name='3D View Panel Location',
        description='Where to place the 3D View panel',
        default='SIDE',
        items=view3d_panel_location_items,
        update=view3d_panel_update)

    def draw(self, context):
        layout = self.layout

        layout.label(text=" ")
        layout.prop(self, "view3d_panel_location_enum", text="Panel Location")
