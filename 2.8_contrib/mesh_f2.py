# updates:
# auto-grab vertex in-place
# fix undo
# remove toggles, add bmesh and normal update
# replace active_object with object. needed for 2.8 multi-editing support
# blah blah
...
def quad_from_edge(bm, edge_sel, context, event):
    ...
    all_edges = [[e for e in edge_sel.verts[i].link_edges if
                 len(e.link_faces) < 2 and e != edge_sel and
                 sum([f in edge_sel.link_faces for f in e.link_faces]) == 0]
                 for i in range(2)]
...
def quad_from_vertex(bm, vert_sel, context, event):
    ...
    for a, b in itertools.combinations(edges, 2):
        other_verts = [v for e in [a, b] for v in e.verts if not v.select]
    ...
    bm.select_history.add(vert_new)
    # adjust uv-map
    if __name__ != '__main__':
        # addon_prefs = context.preferences.addons[__name__].preferences
        if addon_prefs.adjustuv:
...
class MeshF2(bpy.types.Operator):
    ...
    # bl_options = {'REGISTER', 'UNDO'}  # push undo manually

    @classmethod
    def poll(cls, context):
        # don't use active in 2.8
        a = context.object
        b = context.mode == 'EDIT_MESH'
        return a and b
    ...
    def invoke(self, context, event):
        ...
        if addon_prefs.autograb and msm[0]:
            bpy.ops.view3d.select('INVOKE_DEFAULT', False)
        else:
        ...
        if len(v_sel) > 2:
            ...
            quad_from_vertex(bm, v_sel[0], context, event)
            if __name__ != '__main__':
                if addon_prefs.autograb:
                    bpy.ops.transform.translate(
                        'INVOKE_DEFAULT', release_confirm=True)
